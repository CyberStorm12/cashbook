...
function App() {
  ...
  const [isAddLoanTransactionOpen, setIsAddLoanTransactionOpen] =
    useState(false);
  const [isEditLoanTransactionOpen, setIsEditLoanTransactionOpen] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const savedLoanTransactions = localStorage.getItem(
      "cashbook-loan-transactions"
    );
    ...
    if (savedLoanTransactions) {
      ...
      setLoanTransactions(ltxs);
    }
    setIsLoaded(true);
  }, []);

  // Save books to localStorage whenever books change (after load)
  useEffect(() => {
    if (isLoaded) {
  ...
  // Save transactions to localStorage whenever transactions change (after load)
  useEffect(() => {
    if (isLoaded) {
  ...
  // Save loanBooks to localStorage whenever loanBooks change (after load)
  useEffect(() => {
    if (isLoaded) {
  ...
  // Save loanTransactions to localStorage whenever loanTransactions change (after load)
  useEffect(() => {
    if (isLoaded) {
      localStorage.setItem(
        "cashbook-loan-transactions",
        JSON.stringify(loanTransactions)
      );
    }
  }, [loanTransactions, isLoaded]);
  ...
  const addTransaction = () => {
    if (newTransaction.amount && newTransaction.title) {
      const transaction = {
        id: Date.now().toString(),
  ...
  const deleteLoanBook = (loanBookId) => {
    ...
    setLoanTransactions(
      loanTransactions.filter((t) => t.loanBookId !== loanBookId)
    );
  ...
  const editLoanTransaction = () => {
    if (
      ...
      editingLoanTransaction.amount &&
      editingLoanTransaction.title
    ) {
      const updatedLoanTransactions = loanTransactions.map((t) =>
        t.id === editingLoanTransaction.id
          ? {
              ...
              title: editingLoanTransaction.title.trim(),
  ...
  const getLoanBookTransactions = (loanBookId) => {
    return loanTransactions
      .filter((t) => t.loanBookId === loanBookId)
      .sort((a, b) => new Date(b.date) - new Date(a.date));
  };
          ...
          new Notification("Loan Reminder", {
            body: `Reminder for ${
              transaction.type === "loan-given"
                ? "loan given to"
                : "loan taken from"
            } ${transaction.contact || "contact"}: ${transaction.description}`,
            icon: "/favicon.ico",
          });
        }
      }, delay);
  ...
  const importTransactionsFromCsv = (event) => {
    ...
    reader.onload = (e) => {
      ...
      if (missingHeaders.length > 0) {
        alert(
          `Missing required CSV headers: ${missingHeaders.join(
            ", "
          )}. Please ensure your CSV has columns for Type, Amount, Description, Contact, Category, Date, and Created By.`
        );
        return;
      }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (
          !["cash-in", "cash-out"].includes(type) ||
          isNaN(amount) ||
          !description
        ) {
  ...
  const exportTransactionsToCsv = (bookId) => {
    ...
    link.setAttribute(
      "download",
      `${selectedBook.name.replace(/[^a-zA-Z0-9]/g, ".")}_transactions.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const generatePDFReport = async () => {
    try {
      ...
      if (bookTransactions.length > 0) {
        ...
        const tableData = bookTransactions.map((transaction, index) => {
          ...
          return [
            ...
            `${transaction.type === "cash-in" ? "+" : "-"}${formatCurrency(
              transaction.amount
            )}`,
          ];
        });
        ...
        autoTable(doc, {
          head: [
            ["#", "Date", "Time", "Type", "Contact", "Description", "Amount"],
          ],
          body: tableData,
          startY: 135,
          styles: {
      ...
      const filename = `${selectedBook.name.replace(
        /[^a-zA-Z0-9]/g,
        "_"
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    "? This will also delete all transactions in
                                    this book. This action cannot be undone.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
          ...
          <div className="max-w-md mx-auto px-4 py-2">
            <div className="flex justify-around">
              ...
              <Button
                ...
                onClick={() => setCurrentView("loan-books")}
              >
  ...
  if (currentView === "loan-books") {
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        {/* loanbook by qbexel List */}
        <div className="max-w-md mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-medium text-gray-900">
              Your loanbook by qbexel
            </h2>
          </div>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    {loanBook.name}"? This will also delete all
                                    transactions in this book. This action
                                    cannot be undone.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                                  <AlertDialogAction
                                    onClick={() => deleteLoanBook(loanBook.id)}
                                    className="bg-red-600 hover:bg-red-700"
                                  >
                                    Delete
                                  </AlertDialogAction>
              ...
              <div className="space-y-4">
                ...
                <div className="flex space-x-2">
                  <Button
                    variant="outline"
                    className="flex-1"
                    onClick={() => setIsAddLoanBookOpen(false)}
                  >
                    Cancel
                  </Button>
                  <Button
  ...
  if (currentView === "loan-book-detail") {
    ...
    const displayLoanTransactions = getLoanBookTransactions(
      selectedLoanBook.id
    );
      ...
      <div className="min-h-screen bg-gray-50">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl font-bold text-gray-900">
                    {formatCurrency(loanBookStats.netBalance)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Total Given (+)</span>
                  <span className="text-lg font-semibold text-green-600">
                    {formatCurrency(loanBookStats.totalGiven)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Total Taken (-)</span>
                  <span className="text-lg font-semibold text-red-600">
                    {formatCurrency(loanBookStats.totalTaken)}
                  </span>
                </div>
              </div>
            </CardContent>
        ...
        <div className="max-w-md mx-auto px-4 pb-24">
          <p className="text-center text-gray-500 mb-4">
            Showing {displayLoanTransactions.length} entries
          </p>
          ...
          {displayLoanTransactions.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400">
                Add your first loan transaction below
              </p>
            </div>
          ) : (
            <div className="space-y-4">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t">
          <div className="max-w-md mx-auto p-4">
            <div className="flex space-x-2">
              <Dialog
                open={isAddLoanTransactionOpen}
                onOpenChange={setIsAddLoanTransactionOpen}
              >
                <DialogTrigger asChild>
                  <Button
                ...
                <DialogContent className="max-w-sm mx-auto">
                  <DialogHeader>
                    <DialogTitle>
                      Add{" "}
                      {newLoanTransaction.type === "loan-given"
                        ? "Loan Given"
                        : "Loan Taken"}
                    </DialogTitle>
                  </DialogHeader>
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        id="title"
                        value={newLoanTransaction.title}
                        onChange={(e) =>
                          setNewLoanTransaction({
                            ...newLoanTransaction,
                    ...
                    <div>
                      ...
                      <Select
                        value={newLoanTransaction.reminder}
                        onValueChange={(value) =>
                          setNewLoanTransaction({
                            ...newLoanTransaction,
                            reminder: value,
                    ...
                    <div className="flex space-x-2">
                      <Button
                        variant="outline"
                        className="flex-1"
                        onClick={() => setIsAddLoanTransactionOpen(false)}
                      >
                        Cancel
                      </Button>
                      <Button
        ...
        <Dialog
          open={isEditLoanTransactionOpen}
          onOpenChange={setIsEditLoanTransactionOpen}
        >
          <DialogContent className="max-w-sm mx-auto">
            <DialogHeader>
              <DialogTitle>
                Edit{" "}
                {editingLoanTransaction?.type === "loan-given"
                  ? "Loan Given"
                  : "Loan Taken"}
              </DialogTitle>
            </DialogHeader>
            {editingLoanTransaction && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    id="title"
                    value={editingLoanTransaction.title}
                    onChange={(e) =>
                      setEditingLoanTransaction({
                        ...editingLoanTransaction,
                        title: e.target.value,
                ...
                <div className="flex space-x-2">
                  ...
                  <Button
                    variant="outline"
                    className="flex-1"
                    onClick={() => setIsEditLoanTransactionOpen(false)}
                  >
                    Cancel
                  </Button>
                  <Button
          ...
          <CardContent className="p-4">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-gray-900">
                  {formatCurrency(bookStats.netBalance)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Total In (+)</span>
                <span className="text-lg font-semibold text-green-600">
                  {formatCurrency(bookStats.totalIn)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Total Out (-)</span>
                <span className="text-lg font-semibold text-red-600">
                  {formatCurrency(bookStats.totalOut)}
                </span>
              </div>
            </div>
            ...
            >
              ...
              <DialogTrigger asChild>
                <Button
                  className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3"
                  onClick={() =>
                    setNewTransaction({ ...newTransaction, type: "cash-out" })
                  }
                >
                  <Minus className="h-5 w-5 mr-2" />
                  CASH OUT
              ...
              <DialogContent className="max-w-sm mx-auto">
                <DialogHeader>
                  <div className="flex items-center justify-center gap-2 mt-2 mb-2">
                    ...
                    <button
                      type="button"
                      className="inline-flex items-center justify-center w-8 h-8 border border-gray-300 bg-white text-green-600 hover:bg-green-50 rounded-full shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-green-500"
      ...
      >
        <DialogContent className="max-w-sm mx-auto">
          <DialogHeader>
            <DialogTitle>
              Edit{" "}
              {editingTransaction?.type === "cash-in" ? "Cash In" : "Cash Out"}
            </DialogTitle>
          </DialogHeader>
          {editingTransaction && (
            <div className="space-y-4">
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  className={`flex-1 ${
                    editingTransaction.type === "cash-in"
                      ? "bg-green-600 hover:bg-green-700"
                      : "bg-red-600 hover:bg-red-700"
                  }`}
